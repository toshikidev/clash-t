    ...
    class UpdateHandle
    {
        ...
        public void CheckUpdateGuiN(Config config, Action<bool, string> update)
        {
            ...
            AbsoluteCompleted += (sender2, args) =>
            {
                if (args.Success)
                {
                    _updateFunc(false, string.Format(ResUI.MsgParsingSuccessfully, "clashT"));
            ...
            _updateFunc(false, string.Format(ResUI.MsgStartUpdating, "clashT"));
            CheckUpdateAsync(ECoreType.clashT);
        }
