...
namespace clashUpgrade
{
    ...
    {
        private readonly string defaultFilename = "clashT.zip_temp";
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                Process[] existing = Process.GetProcessesByName("clashT");
                foreach (Process p in existing)
                {
                    string path = p.MainModule.FileName;
                    if (path == GetPath("clashT.exe"))
                    {
                        p.Kill();
                        p.WaitForExit(100);
                    }
            ...
            catch (Exception ex)
            {
                ...
                showWarn("Failed to close clashT(关闭clashT失败).\n" +
                    "Close it manually, or the upgrade may fail.(请手动关闭正在运行的clashT，否则可能升级失败。\n\n" + ex.StackTrace);
            }
